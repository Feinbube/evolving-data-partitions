...
namespace DataFieldLayoutSimulation
{
    ...
    {
        public abstract void Mutate(Random random, Arr<int> field, int mutations);
        ...
        protected static int similarNeighbors(int[] field, int x, int y, int w, int h, bool borderAsMatch) { return neighborsWithColor(field[coords(x, y, w)], field, x, y, w, h, borderAsMatch); }

        public static int neighborsWithColor(int value, int[] field, int x, int y, int w, int h, bool borderAsMatch)
        {
            int result = 0;
        ...
        public static int neighborsWithColorHV(int value, int[] field, int x, int y, int w, int h, bool borderAsMatch)
        {
            int result = 0;
            int borderValue = borderAsMatch ? value : -1;
            result += value == at(field, x - 1, y, w, h, borderValue) ? 1 : 0;
            result += value == at(field, x + 1, y, w, h, borderValue) ? 1 : 0;
            result += value == at(field, x, y - 1, w, h, borderValue) ? 1 : 0;
            result += value == at(field, x, y + 1, w, h, borderValue) ? 1 : 0;
            return result;
        }
